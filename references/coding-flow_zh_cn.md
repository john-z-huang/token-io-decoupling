...
本 Flow 用于项目探索、规划、实现、重构、修复、代码/配置/文档物化、构建测试与调试等 Coding 场景。它保留 Input-side Reasoning 与 Primary Output 两类逻辑职责，同时把具体 Coding 规则拆分到独立模块，使互不相关的关注点可以独立演进，避免反复修改同一个共享文件。
