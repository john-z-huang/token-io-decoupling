...
An independent Worker does not continuously stream work logs. Low-decision-density steps such as ordinary file reads, grep results, screenshot changes, scrolling, local analysis, compile-error fixes, and the next command remain in that Worker's own context.
