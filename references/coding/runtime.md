...
When the Input-side Agent's selection criteria in `execution-control.md` indicate reusable context value, the Host may create or reuse a dedicated Session for the optional Context Bootstrap/Refresh responsibility using the active Model Profile's auxiliary binding. That Session writes only the bounded, fingerprinted factual/policy-routing capsule and may be reactivated for delta refresh after relevant project or Skill changes. It is not a mandatory topology split, does not own semantic decisions, and does not replace mandatory instruction loading or fresh verification. Downstream Workers receive only the capsule paths or targeted read-only exposure authorized by the parent.

The Input-side Reasoning Agent decides whether the change's materiality, risk, or explicit user requirement gives independent final verification concrete value. When it selects **Change Verification**, the Host must create one fresh independent Session eligible for that role under the active Profile after Primary Output reaches its implementation checkpoint. The verifier receives the final state, its fingerprint/epoch, and approved verification inputs; it must not inherit the Primary Output implementation history or silently become a repair Worker. Reuse this verifier for subsequent verification slices in the same task conversation, requiring independent re-evaluation of each new fingerprint/epoch and treating earlier verdicts as non-authoritative context only. Create another verifier only for genuinely isolated requirements, such as incompatible environments/snapshots, distinct permission or security domains, or an explicit independent audit.
...
In Single-Session Coding Mode, dual eligibility covers Input-side Reasoning and Primary Output implementation only. It does not authorize the current Session to self-verify a material final change once the independent verifier is selected, nor does it make the current Session the Documentation/Comments & Git Operations Agent or authorize non-trivial Git work. Reuse the selected verifier for later verification epochs; create another only for genuinely isolated requirements. If a selected independent binding or required parameter cannot be created, follow the active Profile's unavailable rule rather than silently falling back to Primary Output self-verification, input-side long-form materialization, or input-side Git operations.
