...
数量确认后，在第一次 Dispatch 前锁定。如果在第一次 Dispatch 前发现确有 replacement 需求，父级只能更新尚未锁定的拟定数量或 allocation；数量未锁定时不得创建、spawn 或 handoff 到新子 Agent。随后父级必须重新确认或更新拟定方案，锁定修订后的数量，并重新满足路线放行条件。只有放行后，child-creation owner 才能在运行时具备安全能力时创建恰好锁定数量的子 Agent；否则阻塞而不能默默改变数量。每个独立职责消耗一个名额；复用不创建新名额。数量锁定后，替换、fork、handoff 到新子 Agent 或增加 verifier 都属于新建：全部禁止执行，也不得为当前指令增加 `child_count`。
