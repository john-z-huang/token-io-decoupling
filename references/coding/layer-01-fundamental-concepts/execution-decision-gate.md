...
For a substantive Claude Code decision, start the CLI in read-only Plan mode with `claude --permission-mode plan`. Plan mode reads files and proposes a plan but makes no edits until the plan is approved. Use Plan mode only through that direct CLI command or another Plan-mode command or API actually exposed to the Agent Session. Plan mode is a **tool permission mode**, not evidence that the Semantic Contract, authorized mutations, user confirmation, or parent Slice Release has passed. After plan approval, apply the general Decision Brief and active workflow's release boundary before editing. If the active Session exposes no Plan-mode command or API, use the general bounded planning process. [Anthropic: common workflows](https://code.claude.com/docs/en/common-workflows), [Anthropic: permissions](https://code.claude.com/docs/en/permissions).
