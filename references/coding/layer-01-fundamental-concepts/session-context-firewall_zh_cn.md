...
Firewall 只限制原始状态进入，不转移语义责任；语义权威性遵循[职责归属](session-role-ownership_zh_cn.md)。临时项目定位事实只属于观察它的 Session，不提升为长期 Contract 状态。
...
使用运行环境实际暴露的本地代码检索／文件读取工具定位符号和少量源代码区间，返回稳定路径与证据指针，而非完整构建日志或仓库递归列表。Codex 自定义 Agent 可以通过经审查的 `sandbox_mode: read-only` 和窄范围 `mcp_servers` 限定职责访问；省略的字段可能继承父级设置，父级实时权限覆盖还可能影响创建后的 Child。本地 Codex Skill 先暴露元数据，选中后才加载 `SKILL.md`，references 则按需读取；只选择当前 Slice 所需的概念 owner。CLI／IDE／Desktop Codex 宿主可共享 MCP 配置，通过 `/mcp` 核实连接；不能把插件或独立 Chat Session 的工具访问当成额外 Codex Child 或权限授权。[OpenAI：Skills 渐进式披露](https://learn.chatgpt.com/docs/build-skills)、[OpenAI：MCP 宿主配置](https://learn.chatgpt.com/docs/extend/mcp)、[OpenAI：自定义 Agent](https://learn.chatgpt.com/docs/agent-configuration/subagents)。

## Claude Code CLI / Claude Desktop 特别优化指令

### 原生有界读取与 MCP 按需发现

本地 Claude Code 中，仓库定位优先使用定向的 `Glob`/`Grep` 与少量 `Read` 范围，然后只向父级返回路径、行号来源和压缩事实；不得把完整递归目录或构建日志灌入父级。MCP Tool Search 默认启用：Claude Code 延迟加载 MCP 工具定义，并在需要时发现工具。如果 `ANTHROPIC_BASE_URL` 指向非 Anthropic 官方主机，Claude Code 通常会禁用 Tool Search，因为多数代理不会转发 `tool_reference` 数据块；只有模型与代理均支持这些数据块时，才在启动新 Session 时使用 `ENABLE_TOOL_SEARCH=true claude` 覆盖该回退行为。Azure 上托管的 Microsoft Foundry 部署会在服务端拒绝 Tool Search，因此环境会强制预先加载，不能用该变量覆盖。`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS` 也会令 Tool Search 保持关闭，且不能由 `ENABLE_TOOL_SEARCH` 覆盖。Tool Search 不可用时，应预期 MCP 工具会预先加载，不要声称实现了延迟加载节省。Desktop Chat 可通过获准的桌面扩展/本地 MCP Server 访问本机资源；远程连接器访问远端服务；两者均须核对具体工具权限，不推断本地 shell、无限制文件系统或独立 Agent。
...
Claude Code 的项目／用户自定义 subagent 可将内联 `mcpServers` 限于子代理，避免无关工具说明占用父级上下文；插件附带的 Agent 定义不一定遵循此字段。检查 Desktop Agent Session 直接暴露的 MCP 工具，只使用已放行 Slice 确需且获授权的工具。[Anthropic：子代理 MCP 范围](https://code.claude.com/docs/en/sub-agents)。
