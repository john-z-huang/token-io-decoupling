...
### Native child profiles and inherited permissions

Current local Codex exposes built-in `default`, `worker`, and `explorer` profiles; a reviewed custom profile under `.codex/agents/` or `~/.codex/agents/` may specialize a released role. Confirm `agents.enabled` and the actual spawn capability before child creation. Resolve each model/effort from custom file, explicit spawn, `[agents]` defaults and parent as documented, and check inherited `sandbox_mode`/`mcp_servers`/`skills.config`. Live parent `/permissions` or CLI sandbox overrides can be reapplied to a spawned child; do not treat a read-only TOML default as an enforcement proof. The runtime's `agents.max_concurrent_threads_per_session` is a simultaneous-thread cap, **not** the conversation's locked total child count. [OpenAI: subagent profiles, precedence and permissions](https://learn.chatgpt.com/docs/agent-configuration/subagents).
...
The actual `Agent` call must still wait for the existing released slice and reserved unbound allocation. A `SubagentStart` hook can inject context but **cannot block creation**, so it is not a substitute for a pre-creation gate. In Claude Desktop, call `Agent` only if the active Session directly exposes and authorizes that built-in tool; otherwise record it as unavailable and block the dependent slice. Confirm the effective model using the runtime owner rather than inferring tool availability from the app identity.

Official references: [custom subagents and frontmatter](https://code.claude.com/docs/en/sub-agents), [SubagentStart hook](https://code.claude.com/docs/en/hooks).
