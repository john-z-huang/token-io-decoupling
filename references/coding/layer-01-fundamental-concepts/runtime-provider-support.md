...
### Desktop agent tool-only boundary

For both ChatGPT Desktop and Claude Desktop, an Agent may execute **only tools actually exposed to and authorized for its current Agent Session**. Never use Computer Use, screenshots plus clicking, simulated mouse/keyboard, accessibility automation, window controls, or shell-driven GUI automation to operate either desktop app, even when such an automation tool is available. Product documentation about a tab, panel, selector, sidebar, or button describes capability only: it is **not** an Agent operation or permission. A desktop feature may be used by the Agent only when the current Session exposes a direct, authorized API, HTTP, CLI, or MCP operation for that exact action. If a necessary step requires Computer Use, immediately stop the entire Skill task and report the blocked step and incomplete work to the user under the absolute rule in the repository instructions; no GUI or Computer Use workaround is allowed. Do not claim an unexposed GUI feature is equivalent to an exposed Agent tool, and do not bypass this boundary via a separate desktop automation helper.
...
In local Desktop Code sessions, the model dropdown, sidebar, and settings are human-facing product UI, **not Agent tools**. The Agent must obtain the effective model, Session state, and permissions only through controls actually exposed to its Session; it must not click or automate those controls, and an inaccessible UI-only change remains unavailable to the Agent. Desktop has no per-session equivalent of CLI `--allowedTools`/`--disallowedTools`. The Desktop Code tab may load local MCP definitions from `claude_desktop_config.json`, but the standalone CLI does **not** read that file automatically; verify or import the intended server only through exposed authorized tools rather than assuming identical server lists. These UI paths are not available as CLI flags in Desktop Chat.
