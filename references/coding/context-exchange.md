...
Freshness is checked against `HEAD`/tree, tracked-delta fingerprint, listed source hashes, relevant untracked project state, and the active task/scope. If any material field differs, the parent reactivates the same Bootstrap Worker for a delta refresh of affected sections before relying on the capsule. If refresh is unavailable, the receiving Worker reads the named authoritative sources directly and treats affected capsule claims as stale. Refresh does not replace a fresh Change Verification Session, and a verifier may use a current bounded capsule without inheriting implementation history or a prior verdict.
