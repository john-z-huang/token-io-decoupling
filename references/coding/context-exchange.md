...
- **Change Verification** receives the final project state or an isolated verification snapshot, a verification-slice/final-state fingerprint or epoch, the Contract and acceptance criteria, changed-scope evidence, and the compressed implementation-feedback summary. The initial verifier Session is fresh and independent of Primary Output; subsequent verification slices in the same task conversation reuse that verifier. When available, the handoff should identify the accumulated full-suite entrypoint or equivalent manifest and its known coverage boundary so the verifier can run it before targeted changed-risk analysis. It must independently re-evaluate each supplied epoch and must not treat an earlier verdict as evidence. It must not modify tracked product/test/documentation files and must return evidence rather than repairs. If it identifies a valuable recurring coverage gap, it reports the precise expected assertion while remaining read-only; the parent routes materialization to Primary Output, then reuses the same verifier for the new epoch and cumulative-suite run.
