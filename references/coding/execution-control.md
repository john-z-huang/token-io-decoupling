...
Normal progress and results use the Host's parent-only return channel, a Control Checkpoint, or the final Worker result. If the Host provides only a generic chat tool, it may be used only when the parent target is fixed and immutable. If the Host cannot guarantee that routing or cannot narrow the Worker tool surface, the parent must treat the Worker as runtime-blocked and must not dispatch it. This feedback-channel rule does not replace or weaken the Progress Signal and Control Checkpoint cadence below.
