...
This module assumes that the caller has already supplied role/session context and the active Semantic Contract. It defines execution control only; it does not define role topology, deployment binding, or context transport.
...
Parallelism is reserved for independent work that will not contend for the same write targets. Tasks with dependencies, shared files, or ordered result relationships are executed sequentially. When multiple independent Agents need reusable state, use a bounded workspace exchange selected by the active workflow rather than parent-generated long summaries.
...
### Three-part task output after analysis

After input-side analysis and any needed reconnaissance are complete, the parent expresses the approved work in exactly three task sections: **Development**, **Documentation**, and **Verification**. These are concise task assignments, not a second plan or a command-by-command recipe. Each applicable section names its role owner and states only the objective, authorized scope/mutations, return conditions, and unreleased boundary needed for that Worker to act safely. Mark a section `Not applicable` with a brief reason when it has no concrete work; do not create a no-op Worker. The deployment checks still determine whether a role is a separate Session or is fulfilled in the current Session.

Each applicable assignment must include all four fields below. A `Not applicable` section needs only the reason:

```text
Development task content — Owner: Primary Output
Objective: ...
Authorized scope/mutations: ...
Return conditions: ...
Unreleased boundary: ...

Documentation task content — Owner: Documentation/Comments & Git Operations (documentation-only slice)
...

Verification task content — Owner: Change Verification
...
```

Keep the three sections in the parent task bundle, but send each independent Worker only its own section plus the shared context it needs. Do not give a Worker another category's instructions as implied future authorization.

- **Development** maps to Primary Output: implement only the approved code/configuration changes and run provisional, focused feedback checks. It does not own documentation, final verification, or Git work.
- **Documentation** maps to the documentation-only slice of the existing Documentation/Comments & Git Operations role: update only the approved developer documentation or comments, preserving bilingual mirrors. This task does not authorize Git work; the parent must release any Git operation separately under its existing gate. It does not own implementation, tests, or verification scripts.
- **Verification** maps to Change Verification: independently assess the approved acceptance criteria against the final state and script every criterion that can be checked deterministically. Run an available accumulated full-suite entrypoint first, then relevant existing verification scripts before considering new coverage. If no existing asset adequately checks a criterion, the verification slice should authorize the smallest deterministic script needed and the verifier should create or update only that scoped asset and run it. For criteria that cannot be scripted objectively, report the smallest relevant evidence instead of inventing a general framework. The verifier does not repair product behavior or documentation, make semantic decisions, or perform Git work.

Always include all three headings in the task output, even when a section is not applicable. Dispatch only concrete work, one released slice at a time; the headings' display order does not override dependencies or release gates. For a material implementation, verification follows the implementation checkpoint, and documentation follows successful verification unless the existing documentation-only fast path applies. Each dispatched Worker reports its compressed result to the parent and stops at its boundary. Only the parent may release a repair, another category, or a later slice. A Worker must not execute work assigned to another category or infer authorization from another Worker's progress. The active workflow supplies any required Dispatch Preview format.
...
### Parent-only feedback channel

`Send a message to parent` denotes the Worker's one and only feedback route. The parent is the root parent Agent that released the current slice; a sibling, an arbitrary thread, or a Worker's own task/thread is never the parent. A Worker MUST NOT select a target thread or use a chat/message interface to contact, instruct, reassign, or coordinate another Agent. `Need` requests the root parent to make a decision or perform orchestration; it does not mean that the Worker has already created, contacted, or re-routed anything.

Normal progress and results use the runtime environment's parent-only return channel, a Control Checkpoint, or the final Worker result. If the runtime environment provides only a generic chat tool, it may be used only when the parent target is fixed and immutable. If the runtime environment cannot guarantee that routing or cannot narrow the Worker tool surface, the parent must treat the Worker as runtime-blocked and must not dispatch it. This feedback-channel rule does not replace or weaken the Progress Signal and Control Checkpoint cadence below.
...
In the standard two-Session interface, this checkpoint is an internal decision point by default. The input-side Agent must use the Worker feedback, the current Contract, and available evidence to make the ordinary continuation, amendment, repair, routing, or stop decision and keep the decision-execution loop running while the goal remains incomplete. A Worker request for guidance, an ordinary ambiguous finding, model weakness, or a parent/worker checkpoint does not by itself require user input. Pause for the user only when the design/process or user instruction explicitly requires a manual stop at that stage, or when the next operation presents a substantial security or safety risk that cannot be safely resolved from the existing Contract. Preserve higher-priority safety, permission, authorization, and unavailable-capability rules; this default does not authorize bypassing them.
...
The verifier is read-only with respect to product code/configuration, documentation, and Git state. Its released slice may authorize writes only to named verification-script/test paths (or one explicitly bounded verification directory for a new script). If no existing asset adequately checks an acceptance criterion that can be tested deterministically, the verifier should add the smallest deterministic check within that write scope and run it; it must not broaden the suite or refactor unrelated tests. If that write changes tracked state, it reports the script paths, criterion coverage, and provisional result, then pauses without claiming final verification for the pre-write fingerprint. The parent supplies the updated scope evidence and final-state fingerprint and re-releases the same verifier to rerun the accumulated suite and the scoped check on that epoch. If the finding requires a product, documentation, or out-of-scope test repair, it reports the gap and precise expected assertion to the parent, which routes that work to the appropriate Worker. Do not create a new verifier for every verification point or coverage gap.
...
5. After the final tree and any documentation checkpoint pass, Input-side Reasoning may release the same role for a commit, push, remote, or Issue/PR Git slice when explicit user/task authority and the required runtime-environment capability are present. The Git slice returns compressed local and remote evidence; it does not make product decisions or perform semantic acceptance.
