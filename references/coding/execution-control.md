...
The Change Verification responsibility owns final change-result verification for a material change. Its fresh independent Agent/Session inspects the final project state and relevant surrounding behavior, checks the complete changed scope and accidental-file status against the changed-scope manifest and Git evidence produced by Documentation/Comments & Git Operations, and runs the appropriate holistic evidence-producing checks, such as integration, regression, cross-module, system, or end-to-end tests. Fresh means independent judgment and no inherited Primary Output implementation history or prior verifier verdict; it does not require contextlessness, and a current bounded factual/routing capsule may be used. It does not perform non-trivial Git queries or operations. Normally use one verifier Session per cohesive material final-state fingerprint (epoch), covering the acceptance matrix and all appropriate checks for that immutable state. It reports compressed evidence, failures, coverage gaps, and residual risks; it does not modify product code, tests, documentation, or configuration and does not repair findings.

Reuse that verifier for targeted Evidence-on-Demand or clarification while the final-state fingerprint is unchanged. A material repair changes the fingerprint and requires a new fresh verifier; a non-material rerun on the identical state does not create another verifier merely because a check was pending.
...
The input-side responsibility owns semantic acceptance: whether the user's goal is satisfied, the Semantic Contract is implemented, business/compatibility constraints remain intact, and the risks reported by Change Verification, documentation checks, and Git-operation evidence are acceptable. It also decides whether a change is material enough to require a fresh verifier, whether post-verification documentation/comment work is needed, and whether a Git slice is authorized for the current stage.
...
3. Input-side Reasoning maps each acceptance criterion to verifier evidence and chooses `Continue`, `Amend`, or `Stop`. On failure, it releases only a narrowly scoped repair slice to Primary Output and then requires a fresh verification pass for the repaired final state. Documentation/comment and dependent remote Git slices must not start while a material verification issue remains unresolved.
