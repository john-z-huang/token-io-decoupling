...
The Change Verification Agent must use a fresh independent Session when selected so it does not inherit the Primary Output implementation history. Its authorized verification slice is read-only with respect to product code, tests, documentation, configuration, and Git state. It consumes the changed-scope manifest and Git evidence supplied by the Documentation/Comments & Git Operations Agent, then verifies the resulting content and behavior; it does not perform non-trivial Git queries or operations. Disposable test/build outputs may be isolated by the Host. It must not recursively delegate. A material repair returns to Primary Output through the parent Input-side Agent and requires a fresh verification pass for the new final state.
