...
3. **模式与任务记录。** 确认[模式检查点](../layer-02-workflow-concepts/mode_zh_cn.md)只放行 `Single-Agent Coding`。核对[模式确认](../layer-01-fundamental-concepts/delegation-mode-confirmation_zh_cn.md)、[数量门禁](../layer-01-fundamental-concepts/delegation-mode-count-gate_zh_cn.md)和[状态记录](../layer-01-fundamental-concepts/delegation-state-record_zh_cn.md)：`gate_status: released`、`child_count: 0`、`allocations: []`，且没有未解决的能力阻塞。后续根指令实质改变委派时，应用[模式重新进入](../layer-01-fundamental-concepts/delegation-mode-reentry_zh_cn.md)。本路线不得创建、派生、发送消息、移交或模拟额外 Agent 或 Session。
