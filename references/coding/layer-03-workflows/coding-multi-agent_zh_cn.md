...
3. **模式、数量与记录。** 确认[模式检查点](../layer-02-workflow-concepts/mode_zh_cn.md)只放行 `Multi-Agent Coding`。核对[模式确认](../layer-01-fundamental-concepts/delegation-mode-confirmation_zh_cn.md)、[数量门禁](../layer-01-fundamental-concepts/delegation-mode-count-gate_zh_cn.md)、[状态记录](../layer-01-fundamental-concepts/delegation-state-record_zh_cn.md)和[模式重新进入](../layer-01-fundamental-concepts/delegation-mode-reentry_zh_cn.md)：会话模式和正整数数量已明确选择，或在 15 秒超时后采用默认值并锁定；`gate_status: released`；恰好 `child_count` 个已锁定计划分配各自具有职责、切片、范围、修改、返回条件、生命周期和显式 `write_content_memo`。本次会话后续指令沿用已锁定模式/数量；拓扑冲突时阻塞依赖工作，直到新会话。禁止条件、缺失的记录或不可用能力会阻塞本路线；不得静默更改模式或数量。
