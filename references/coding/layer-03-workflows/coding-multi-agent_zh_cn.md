...
分配任何子代理前，父级直接分析指令、确认项目环境，并核查定义任务所需的代码细节。实质性事实不足时，由父级执行有界只读侦察，完成上述确认后才能分配。第一次实质性派发前，除非任务简单、局部、低风险、明显、可逆且可机械验证，否则先形成简洁的 Decision Brief。Worker 随后执行精确的已放行指令，将冲突或新发现的事实返回父级重新决策。
