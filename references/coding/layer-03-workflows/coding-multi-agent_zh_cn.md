...
3. **模式、数量与记录。** 确认[模式检查点](../layer-02-workflow-concepts/mode_zh_cn.md)只放行 `Multi-Agent Coding`。核对[模式确认](../layer-01-fundamental-concepts/delegation-mode-confirmation_zh_cn.md)、[数量门禁](../layer-01-fundamental-concepts/delegation-mode-count-gate_zh_cn.md)、[状态记录](../layer-01-fundamental-concepts/delegation-state-record_zh_cn.md)和[模式重新进入](../layer-01-fundamental-concepts/delegation-mode-reentry_zh_cn.md)：根模式与准确的正整数数量已确认；`gate_status: released`；恰好 `child_count` 个已锁定计划分配各自具有职责、切片、范围、修改、返回条件、生命周期和显式 `write_content_memo`。新根指令或实质委派变化必须重新进入门禁。禁止条件、缺失的记录或不可用能力会阻塞本路线；不得静默更改模式或数量。
