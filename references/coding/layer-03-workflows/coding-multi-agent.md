...
Use the inventory and shared runtime rules from the [Environment capability inventory](../layer-02-workflow-concepts/environment-capability-inventory.md). Continue only when it exposes the capabilities required by the task record and released slice. A Worker returns only to its direct parent and cannot create/manage Agents or contact arbitrary threads. The following profile bindings apply only to the Local Codex branch. They are capability requirements for the route's role assignments, not default availability assumptions about ChatGPT Work or Standard ChatGPT: Primary Output `gpt-6-luna`/`medium`; Change Verification `gpt-6-luna`/`medium`; Documentation/Git `gpt-6-luna`/`medium`; Context Bootstrap/Refresh `gpt-6-luna`/`medium`, with `low` permitted for deterministic refreshes. The Local Codex capability inventory must verify the exposed model identity and reasoning parameter for each assigned binding. If a required binding is unavailable or unknown, record it in `unavailable_capabilities` and block the dependent slice/route. Do not silently substitute a model, reasoning effort, Session, tool, permission, or mode/count; selecting a different route requires re-entering the applicable gate and obtaining new explicit confirmation. Raise only the affected slice to `high` when concrete complexity or repeated failure/blockage evidence warrants it, then return to the normal tier. Escalate to `max` only when `high` was insufficient and repeated failure/blockage continues, then return to the normal tier.
