...
对于 `context/context-bootstrap/` 下的 capsule，使用：
...
- 使用 `<primary-worktree>/.token-io-decoupling/context/` 作为 Context Exchange Root。父 Agent 独占维护 `<root>/INDEX.md`，将每个已分配或历史 Context ID 映射到目录，并记录最小 routing 状态。
- 已分配 Worker 使用文件化交换前，父 Agent 为其准备 `<root>/<worker-context-id>/` 并提供准确路径。Worker 只能在该目录内写入；除非父 Agent 为具体 handoff 或依赖关系点名文档，否则不得访问其他 Worker 目录。
