...
Live use of this deployment has exercised Skill discovery and Runtime resolution, host identity resolution, Sonnet-tier Single-Session Coding Mode, and vendor-neutral Core loading. The checks that depend on a Haiku-tier auxiliary dispatch or on a normal two-Session Primary Output dispatch have not been exercised yet: report those capabilities as unverified rather than assumed, and do not present the whole deployment as uniformly smoke-tested.
