...
- **Change Verification**: when selected, use a fresh independent `gpt-5.6-luna` Session for final change-result verification.
- **Documentation/Comments**: when selected, use a separate `gpt-5.6-luna` Session for post-verification documentation and code-comment materialization only.
...
Single-Session Coding Mode preserves the current deployment behavior for implementation: the current Luna directly performs project exploration, implementation, debugging, provisional focused checks, and implementation output rather than delegating ordinary work to another Luna merely to preserve a two-role topology. For a material functional change, the Input-side Agent still creates a fresh Change Verification Luna; it may create a separate Documentation/Comments Luna after verification. Because the current Session also owns high-value reasoning, its ordinary substantive implementation uses `reasoning_effort=xhigh`.
...
- A selected Documentation/Comments Luna defaults to `reasoning_effort=high` for bounded developer documentation and code-comment materialization after verification. Its write scope excludes functionality and tests; it must not be used to compensate for failed verification.
