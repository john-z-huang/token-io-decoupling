...
Before downstream use, compare the capsule's `HEAD`/tree, tracked-delta fingerprint, listed source hashes, relevant untracked state, and active task/scope. After a material relevant change, reuse the same Bootstrap Worker for a delta refresh of affected sections, or read the named authoritative files directly until refresh completes. For a material task, reuse one independent verifier for Evidence-on-Demand and every later verification epoch; require independent re-evaluation for each new final-state fingerprint and never treat the prior verdict as evidence. Create another verifier only for genuinely isolated requirements. Routine delivery read-back stays with the delivery slice unless a concrete external-risk criterion explicitly selects independent delivery verification.
