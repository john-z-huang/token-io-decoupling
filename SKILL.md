...
Coding delegation and Session-topology rules are split into atomic references: the state record, mode/count gate, and conditional child dispatch/lifecycle policy. The Mode checkpoint composes and validates them before route selection; the parent-controlled record stores a conversation-wide mode/count decision and current task state.
...
3. Complete the Mode checkpoint using that inventory. On this conversation's first Coding directive, use an explicit user choice or ask once, wait 15 seconds, and default to Multi-Agent with one child if no clear reply is observable. Confirm the task control record contains a released mode and its required state; later directives reuse the locked mode/count.
...
- Computer Use is categorically forbidden: never control a computer, browser, or desktop app through its graphical interface — no screenshots to locate controls, mouse or keyboard simulation, clicking, accessibility APIs, window automation, remote-desktop control, or GUI scripting — and do not route such interaction through an API, CLI, MCP server, helper, or another Agent. Treat this as a blacklist rather than a list of allowed channels: every other tool, command, or capability actually exposed to and authorized for the current Session remains permitted. If a required step needs a forbidden operation, stop and report the blocker instead of attempting, delegating, or skipping it.
- Before route selection or any delegation/Session-topology work, the Mode checkpoint must compose and validate the delegation references; those references own conversation-level mode selection with a 15-second fallback, child counts, creation, allocation, reuse, replacement, exceptions, and limits at their respective atomic boundaries.
