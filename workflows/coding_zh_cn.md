...
当仓库文本、开发工具输出、实现、测试、文档或 Git 状态是主要工作状态时，使用本工作流。本文件只负责 Coding 路线选择和完成索引。选定的 `exist-workflow` 文档负责组合执行所需的 references 与检查点。根指令模式和委派规则只由委派 policy 负责；任务控制记录保存当前状态。
